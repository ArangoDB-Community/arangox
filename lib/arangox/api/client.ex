defmodule Arangox.API.Client do
  ...
  ## Options every operation accepts
    ...
    * `:database` - the database to run a database-scoped operation against.
      Falls back to the pool's own `:database`, and then to `_system`.
      Server-global operations ignore it and never receive a `/_db/` prefix.
  ...
  A successful call returns `{:ok, body}` — the decoded response body, not a
  response struct. A success that carries no body (`204 No Content`) returns
  `{:ok, nil}`.

  Any error status returns `{:error, %Arangox.Error{}}`, carrying the HTTP
  ...
  @server_scope_option :_arangox_server_scope
  ...
  #   * `:response_type` — `:body` (default), or `:revision` for a HEAD
  #     operation whose return value is the `etag` header
  #   * `:database_scope` — `:server` for an address the server defines without
  #     a database prefix; omitted for an ordinary database-scoped operation
  ...
  def request(conn, spec) do
    ...
    response_type = Keyword.get(spec, :response_type, :body)
    ...
    with {:ok, path} <- path(segments),
      ...
      response =
        Arangox.request(
          ...
          forwarded(opts, declared, Keyword.get(spec, :database_scope))
        )

      do_response(response, response_type)
    end
  end
  ...
  @spec path(list) :: {:ok, binary} | {:error, Error.t()}
  ...
  @spec checked(term) :: {:ok, binary} | {:error, Error.t()}
  ...
  @spec control_byte?(binary) :: boolean
  ...
  @spec with_query(binary, keyword, [{binary, term}], keyword) ::
  ...
  @spec offered(keyword, keyword) :: [{binary, term}]
  ...
  @spec append_query(binary, [{binary, term}]) :: binary
  ...
  @spec forwarded(keyword, keyword, :server | nil) :: keyword
  defp forwarded(opts, declared, database_scope) do
    opts = Keyword.drop(opts, [@server_scope_option, :headers | Keyword.keys(declared)])

    if database_scope == :server do
      opts
      |> Keyword.delete(:database)
      |> Keyword.put(@server_scope_option, true)
    else
      opts
    end
  end
  ...
  @spec check_header(term) :: :ok | {:error, Error.t()}
  ...
  @spec headers_with_media([{term, term}], binary | nil) :: [{term, term}]
  ...
  # be lowercase.
  @spec header(Arangox.headers(), binary) :: binary | nil
  defp header(headers, name) do
    Enum.find_value(headers, fn {key, value} -> if named?(key, name), do: value end)
  end

  @spec has_header?([{term, term}], binary) :: boolean
  defp has_header?(headers, name) do
    Enum.any?(headers, fn {key, _value} -> named?(key, name) end)
  end

  @spec named?(term, binary) :: boolean
  defp named?(key, name), do: String.downcase(to_string(key)) == name

  ## The response

  @spec do_response(
          {:ok, Arangox.Request.t(), Response.t()} | {:error, term},
          :body | :revision
        ) :: {:ok, Arangox.body()} | {:error, term}
  defp do_response({:ok, _request, %Response{body: body}}, :body), do: {:ok, body}

  # A `HEAD` request has no body at all; it returns the document revision,
  # which the server sends as a quoted `etag`. Handing back an empty body
  # would make the operation useless.
  defp do_response({:ok, _request, %Response{headers: headers}}, :revision) do
    {:ok, headers |> header("etag") |> unquote_etag()}
  end

  defp do_response({:error, exception}, _response_type), do: {:error, exception}

  @spec unquote_etag(binary | nil) :: binary | nil
  ...
  @spec refuse(binary) :: {:error, Error.t()}
  defp refuse(message), do: {:error, %Error{message: message}}

  @spec smuggles?(term) :: boolean
