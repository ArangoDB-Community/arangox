...
- The transaction identifier is a bearer capability, redacted from inspection and logs like authentication material. Relevant here because the fix keeps it in connection state longer than the released version did, and retention must not widen exposure.
- Sibling learning in the same module, orthogonal failure mode: [DBConnection's :timeout is a deadline, not a duration you can reuse downstream](../architecture-patterns/dbconnection-timeout-is-a-deadline-not-a-duration.md) — deadline arithmetic rather than premature state clearing, but the same lesson family: a DBConnection driver's local bookkeeping must not run ahead of ground truth.
</content>
